...
from screenlets.options import IntOption, StringOption, BoolOption, ColorOption, FontOption, ListOption, StringOption
...
class HDUsageScreenlet(screenlets.Screenlet):
	...
	def __init__(self, **keyword_args):
		...
		self.add_option(StringOption('HDUsage Options', 'openCommand', self.openCommand, 'Open command', 'Command used to browse the drive'))
	...
	def updateDriveInfo(self):
		...
		#self.dfInfo = os.popen( "cat temp.txt" ).readlines()
	...
	def rebuildDriveList(self):
		df = os.popen("df -B 1 2>&1").readlines()
		self.driveList = []
		for line in df:
			if "Filesystem" and "Use%" in line:
				continue
				
			# Device-name 1B-blocks Used Available Use Mounted-on
			# 0           1         2    3         4   5
			tmp = re.findall("^(\S*)\s+(\d*)\s+(\d*)\s+(\d*)\s+([\d%]*)\s+(.*)$", line.strip())
			self.driveList.append(tmp)
		return True
	...
	def get_button(self, x, y):
		""" Find out which drive was clicked from the position given """
		if not self.mouseInside:
			return False
		
		#self.mousex >= xoffset and self.mousex < xoffset+self.barWidth and self.mousey >= yoffset and self.mousey < yoffset+self.barHeight:
	...
	def on_draw(self, ctx):
		...
		#ctx.translate( 5+(x*(self.barWidth+5)), 5+(y*(self.barHeight+5)) )
			...
			#Draw background bar
			perc = (self.barWidth/100.0)*float(self.devices[i][4][:-1])
			...
			# this is the percentage threshold at which colour 2 should become the main colour
			#
			colThreshold = 70
			#
			##################################################
			...
			#Draw foreground bar
			realPerc = float(self.devices[i][4][:-1])
			if realPerc < colThreshold:
				realPerc = (realPerc*colThreshold/100)
				if realPerc > 0.0:
					colorPerc = ((realPerc*2.0)/100.0)
				else :
					colorPerc = 0.0
				r = self.barColorFG[0] + (self.barColorFG2[0] * colorPerc)
				g = self.barColorFG[1] + (self.barColorFG2[1] * colorPerc)
				b = self.barColorFG[2] + (self.barColorFG2[2] * colorPerc)
				a = self.barColorFG[3] + (self.barColorFG2[3] * colorPerc)
			else :
				realPerc = ( realPerc * (100-colThreshold)/100)
				colorPerc = 1.0-(realPerc/100.0)
				r = self.barColorFG2[0] + (self.barColorFG[0] * colorPerc)
				g = self.barColorFG2[1] + (self.barColorFG[1] * colorPerc)
				b = self.barColorFG2[2] + (self.barColorFG[2] * colorPerc)
				a = self.barColorFG2[3] + (self.barColorFG[3] * colorPerc)
